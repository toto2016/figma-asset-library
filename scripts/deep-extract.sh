#!/bin/bash
# 高价值文件深度 Token 提取
# 用法: FIGMA_TOKEN=xxx bash scripts/deep-extract.sh
# 提取 5 个高价值库文件的完整组件属性 + 样式定义
...
HIGH_VALUE_FILES = [
    {"fileKey": "OMp0bfNpPE8oHvRGAIrpvU", "name": "Design library - Mobile", "project": "designops"},
    {"fileKey": "jIrRokAq33eq1ifxOtRFKS", "name": "Merchant Centre Library", "project": "designops"},
    {"fileKey": "fvIH2yRyQuqVMRKcmcwXYt", "name": "open-peeps (library)", "project": "designops"},
    {"fileKey": "Q8bFuMuXHKLX6X9S4IcVYU", "name": "Gesture Icons (library)", "project": "designops"},
    {"fileKey": "vUCA0RnhjKYt6XvJat6tdP", "name": "Happy flow All countries", "project": "flow-library"},
]
...
def extract_file(finfo):
    ...
    for p in pages:
        ...
        for c in children:
            ...
            if sub_children:
                node["childCount"] = len(sub_children)
                node["childNames"] = [sc.get("name", "?") for sc in sub_children[:20]]
            page_info["children"].append(node)
    ...
    if comp_node_ids:
        ...
        for batch_start in range(0, len(comp_node_ids), batch_size):
            batch = comp_node_ids[batch_start:batch_start + batch_size]
            ids_param = ",".join(batch)
            nodes_data = fetch_json(
                f"https://api.figma.com/v1/files/{fkey}/nodes?ids={ids_param}"
            )
            if nodes_data and "nodes" in nodes_data:
                for nid, ndata in nodes_data["nodes"].items():
                    ...
                    result["componentDetails"].append({
                        ...
                        "primaryAxisSizingMode": doc.get("primaryAxisSizingMode"),
                        "counterAxisSizingMode": doc.get("counterAxisSizingMode"),
    ...
    if style_node_ids:
        ...
        if style_nodes and "nodes" in style_nodes:
            for nid, ndata in style_nodes["nodes"].items():
                ...
                if doc.get("fills"):
                    detail["fills"] = doc["fills"]
                if doc.get("style"):
                    detail["textStyle"] = doc["style"]
                if doc.get("effects"):
                    detail["effects"] = doc["effects"]
            ...
            "variables": [
                {
                    ...
                    "valuesByMode": v.get("valuesByMode", {}),
                    "scopes": v.get("scopes", [])
                }
...
for finfo in HIGH_VALUE_FILES:
    ...
    if result:
        safe_name = finfo["fileKey"]
        out_path = f"{output_dir}/{safe_name}.json"
        with open(out_path, "w") as f:
            json.dump(result, f, ensure_ascii=False, indent=2)
        all_results.append(result)
        comps = result.get("publishedComponents", 0)
        styles = result.get("publishedStyles", 0)
        vars_count = result.get("variables", 0)
        comp_details = len(result.get("componentDetails", []))
        print(f"  💾 已保存 → {out_path}")
        print(f"     组件: {comps} (详情: {comp_details}), 样式: {styles}, 变量: {vars_count}")
